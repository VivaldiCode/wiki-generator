...
class Manifest:
    ...
    def provenance(self) -> tuple[str | None, str | None]:
        """Which client and model wrote the wiki this manifest describes."""
        return self.data.get("client"), self.data.get("model")

    def record_provenance(self, client: str, model: str) -> None:
...
def provenance_warning(config: WikiConfig) -> str | None:
    """Why an apparently unchanged wiki is about to be regenerated in full.

    A page's fingerprint includes the client and the model, because a page
    written by one is not interchangeable with a page written by another. That
    is correct, and silently invisible: the run just regenerates every page with
    no stated reason, which reads as a broken cache rather than a deliberate
    one. Said before the first paid call — including under --dry-run, which is
    where the decision to spend is actually made.
    """
    was_client, was_model = Manifest(config.output_path / MANIFEST_NAME).provenance()
    if not was_client and not was_model:
        return None  # written before provenance was recorded
    if (was_client, was_model) == (config.client, config.model):
        return None
    return (
        f"This wiki was generated by {was_client}/{was_model}; this run uses "
        f"{config.client}/{config.model}. Every page will be regenerated — pages "
        f"are not shared between clients or models. To continue the existing "
        f"wiki instead, run with --client {was_client} --model {was_model}."
    )
...
class WikiGenerator:
    ...
    async def generate(self, specs: list[PageSpec]) -> GenerationReport:
        ...
        self.manifest.record_provenance(self.config.client, self.config.model)
        self.manifest.save()
